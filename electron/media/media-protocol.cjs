...
function createMediaProtocolHandler(mediaLibrary, brandAssets = null, mediaJobs = null) {
  return async (request) => {
    ...
    try {
      ...
      if (
        !/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,127}$/.test(clipId) ||
        !["clip", "thumbnail", "brand", "job-thumbnail"].includes(kind) ||
        (kind === "brand" && !brandAssets) ||
        (kind === "job-thumbnail" && !mediaJobs)
      ) {
        return new Response("", { status: 404 });
      }
      const filePath =
        kind === "clip"
          ? mediaLibrary.resolveClipPath(clipId)
          : kind === "thumbnail"
            ? mediaLibrary.resolveThumbnailPath(clipId)
            : kind === "brand"
              ? brandAssets.resolve(clipId).filePath
              : mediaJobs.resolveThumbnailArtifact(clipId);
      ...
      const headers = new Headers({
        ...
        "Cache-Control": ["thumbnail", "job-thumbnail"].includes(kind) ? "private, max-age=3600" : "no-store"
      });
