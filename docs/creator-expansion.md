...
| Brand composition     | Recoverable templates/assets and immutable render snapshots             | template/asset stores, render plan, FFmpeg worker           | Studio → Templates and Projects                                               | brand assets, projects, media worker    | Build/apply/export templates; render reviewed assets            |
| Localization/voice    | Language drafts, reviewed WAV previews, consent/likeness records         | provider service, encrypted vault, local/cloud adapters     | Studio → Projects                                                             | provider, local voice, renderer         | Translate, authorize, preview, review, remove                   |
| Shopify analytics     | Derived definitions, freshness, comparisons, observations               | central analytics report and read-only Advisor projection   | Analytics                                                                     | analytics, renderer                    | Inspect/export supported aggregate metrics                     |
| Contract foundations  | UI-free collaboration and developer-platform contracts                  | strict schemas, authorization, integrity, crypto helpers    | Not exposed                                                                   | collaboration/developer contract suites | No hosted workflow until owner infrastructure exists           |

The seven application sidebar rows remain unchanged. Projects is the fourth
Content Studio tab; Templates is the fifth. Library, Create clips, Publishing,
and every existing media workflow remain available. Existing media jobs are
annotated as `clip_generation`; none is promoted, hidden, or rewritten into a
Project.
...
| G. Brand templates                         | Official Brand Template APIs and screenshot categories  | Recoverable template versions, builder/preview, portable packages, branded captions/layouts, text/CTA, fades, and snapshotted logo/music/intro/outro assets             | Partial: custom fonts, advanced motion and generated assets deferred     | Yes                                    | Optional for generated assets             | Validate/copy assets; immutable queued snapshot                                  | Template/asset stores; very high   | **2 implemented:** package round trips and preview/render assets remain deterministic |
| H. Captions/transcripts                    | Source transcript API and caption tools                 | Local/cloud transcription, SRT/VTT import, corrections, rebasing, styling, translation variants, SRT and burned modes                                                  | Partial: RTL/font packages and full diarization remain deferred          | Yes                                    | Optional transcription/translation        | Transcript separate from style; provider consent                                | FFmpeg/fonts/providers; high       | **2/4 foundation implemented:** cues remain monotonic across edits and renders       |
| I. Speech/audio cleanup                    | Speech cleanup/enhancement features                     | Silence/activity signals plus bounded, reviewed local cleanup/enhancement ranges; original audio preserved                                                            | Partial: specialist restoration models and granular preview deferred     | Yes                                    | Optional enhancement provider             | Every enabled range is explicit and removable                                   | FFmpeg/audio DSP; high             | **3 foundation implemented:** immutable jobs preserve reviewed treatment            |
| J. B-roll/custom media                     | AI/stock B-roll and custom upload                       | Library search plus reviewed, fingerprint-verified user-owned B-roll track snapshots                                                                                   | Partial: official stock and generative acquisition deferred              | Yes for owned media                    | Official stock/generative APIs            | No silent generation/download; source identity verified                         | Composition graph; very high       | **3 foundation implemented:** local insertion remains reviewable and path-free       |
| K. Text/headlines/overlays/transitions/SFX | Editor overlays, AI suggestions and transitions         | Timed text/CTA overlays, bounded per-cut fades, caption tracks, and managed local sound-effect mixing                                                                  | Partial: animated overlays and generated suggestions deferred            | Yes for manual/local SFX               | Optional generation                       | User assets are validated and snapshotted                                       | Composition graph; very high       | **2/3 foundation implemented:** approved snapshot deterministically drives render    |
| L. Music                                   | Music controls                                          | Managed local music assets with immutable job snapshots and FFmpeg mixing                                                                                              | Partial: detailed trim/loop/duck/fade UI and stock catalog deferred      | Yes                                    | Official stock APIs optional              | Rights remain user responsibility; no unlicensed catalog                        | Audio mixing; high                 | **2 foundation implemented:** selected local music is validated and reproducible     |
| M. Voiceover/translation/dubbing           | Voice and localization tools                            | Reviewed language variants and consented provider/local WAV previews; provider-scoped voices, multi-speaker drafts, likeness authorization/removal, mix/replace render | Partial: automated diarization and full-length timing adaptation         | Yes for configured local runtimes      | Declared cloud speech providers           | Exact disclosure, identity authorization, provenance, deletion                  | TTS/diarization; very high         | **4 foundation implemented:** no preview renders before explicit human review        |
| N. Video enhancement                       | Upscale and visual adjustment                           | Reviewed local HD-frame resize with explicit “pixel resize, not recovered detail” labeling                                                                             | Partial: restoration/upscale models and visual adjustment suite deferred | Yes, bounded                           | Optional upscale provider                 | Never claims recovered detail; no silent provider use                           | FFmpeg/GPU/provider; high          | **4 foundation implemented:** before/after operation remains truthfully labeled      |
...
| V. API/webhooks/automation                 | Official project/clip/transcript/social APIs            | UI-free v1 scopes/routes/schemas, OpenAPI draft, token/cursor credentials, idempotency, limits, signed-webhook, abuse/audit, and hosted-record contracts              | No listener, identity, database, token UI, dispatcher, or public service | Optional future service                | Hosted delivery for webhooks              | Disabled; secrets one-time/sealed; tenant scope and replay defense              | API server/backend; very high      | **8 contract foundation implemented:** hosting waits for owner infrastructure        |
...
| X. Juanito                                 | ProduDash-specific advisor scope                        | Current project/render readiness/source/revision/job details plus setup, Library, provider and evidence-backed Shopify analytics summaries                             | Future official publishing/social tools wait for connected sources      | Yes tool execution; model may be cloud | Selected advisor provider                 | Read-only, consent-aware, business-scoped, no edit/approve/delete/upload/render | Advisor allowlist; medium          | **Each phase:** bounded tool tests continue to prove mutation prohibition            |
