...
| D. Semantic search                         | Search-oriented clip discovery                          | Library/project filename, tags, collection and local transcript text                                                                                                   | Missing natural-language embeddings/timestamp results                    | Yes                                    | Optional embeddings provider              | Never upload whole library automatically                                        | Index/embedding model; high        | **3:** cancellable reindex with provenance and why-match                            |
...
- **Implemented Phase 3 foundation:** recoverable media-index v2 search
  documents with local-metadata provenance, deterministic synonym-aware ranking
...
- **Implemented Phase 4 foundation:** every approved render creates three
  ...
  built-in or authorized custom-voice WAV previews only after exact provider, model, text, voice and
  ...
  installed readiness and never downloads runtimes or model weights. Loopback
  OpenAI-compatible endpoints may explicitly declare `speech_generation`.
